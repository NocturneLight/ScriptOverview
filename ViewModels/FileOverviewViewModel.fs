...
type FileOverviewViewModel() =
    ...
    let FormatDocument (file: Run seq seq option) (format: FORMAT) =
        try
            file
        ...
        with
        | ex ->
            _Log.Error(ex, "An error occurred with trying to format the document.")
            
            // Return nothing if we catch an error as we must return something.
            Seq.empty
    ...
    let SaveToNaniScript(sender: Window) =
        ...
        with
        | ex ->
            _Log.Error(ex, "An error occurred trying to split the document into individual .nani files.")
    ...
    let WriteActorsToFile(sender: Window) =
        ...
        with
        | ex ->
            _Log.Error(ex, "An error occurred trying to parse the document for actors.")
    ...
    do
        ...
        with
        | ex ->
            _Log.Error(ex, "An error occurred while trying to create and set Reactive Commands.")
    ...
    member this.InitializeView(file: IStorageFile) =
        ...
        with
        | ex ->
            _Log.Error(ex, "An error occurred during Regex matching.")
        ...
        with
        | ex ->
            _Log.Error(ex, "An error occurred trying to open the word document.")
